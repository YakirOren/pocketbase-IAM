...
export function GroupShow() {
  ...
  const { query } = useShow({ resource: "iam_groups", id });
  ...
  const { result: groupPoliciesResult } = useList({
    ...
    filters: [{ field: "group", operator: "eq", value: id }],
    meta: { expand: ["policy"] },
  });
  const attachedPolicies: AttachedPolicy[] = (groupPoliciesResult.data ?? []).map((r: Record<string, unknown>) => {
  ...
  const members: AttachedEntity[] = (groupUsersResult.data ?? []).map((r: Record<string, unknown>) => {
  ...
  const effectivePerms = computeEffectivePermissions(
    attachedPolicies
      ...
      .map((p) => ({
        policyName: p.label,
        document: p.document!,
        source: `Group: ${(record?.name as string) ?? ""}`,
      }))
  );
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <EntityChipList
        ...
        onAdd={(policyId) =>
          createJoin({ resource: "iam_group_policies", values: { group: id, policy: policyId } })
        }
        onRemove={(joinId) =>
      ...
      <EntityChipList
        ...
        addLabel="Add Member"
        onAdd={(userId) =>
          createJoin({ resource: "iam_group_users", values: { group: id, user: userId } })
        }
        onRemove={(joinId) =>
